...
class HyphenBridge implements IBridgeService {
    ...
    async getHyphenSupportedToken(chainId: number): Promise<any> {
        try {
            const supportedTokenurl = this.apiRequestUrl(config.hyphenSupportedTokenEndpoint, {
                networkId: chainId
            });
    
            const response: any = await fetch(supportedTokenurl).then(res => res.json()).then(res => res);
            return response.supportedPairList;
        } catch (error){
    ...
    async getExitCost(exitParams: ExitParams): Promise<number> {
        try {
            ...
            const response: any = await fetch(transferFeeUrl).then(res => res.json()).then(res => res);
    ...
    async getDepositCost(depositParams: HyphenDepositParams): Promise<ethers.BigNumber> {
        try {
            ...
            const depositGasSpend = await this.transactionServiceMap[depositParams.fromChainId].networkService.ethersProvider.estimateGas(
                {
            ...
            let tokenPriceInUsd = await this.tokenPriceService.getTokenPrice(config.NATIVE_TOKEN_SYMBOL[depositParams.fromChainId]);
